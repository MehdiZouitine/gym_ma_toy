...
class World:
    ...
    def reset(self):
        ...
        self.agents = {
            f"agent_{i+1}": Agent(id_elem=(i + 1), position=agents_pos[i]) for (i, pos) in enumerate(agents_pos)}

        self.targets = [Target(position=pos)
    ...
    def _update_action(self, action, agent_id):
        ...
        if action == ACTIONS['UP']:
            if agent.position[0] - 1 > 0 and self.map[agent.position[0] - 1, agent.position[1]] == 0:
        ...
        if action == ACTIONS['DOWN']:
            if agent.position[0] + 1 < self.size and self.map[agent.position[0] + 1, agent.position[1]] == 0:
                self.map[agent.position[0] + 1, agent.position[1]] = 1
                self.map[agent.position[0], agent.position[1]] = 0
                agent.position = (agent.position[0] + 1, agent.position[1])

        if action == ACTIONS['LEFT']:
            if agent.position[1] - 1 > 0 and self.map[agent.position[0], agent.position[1] - 1] == 0:
                self.map[agent.position[0], agent.position[1] - 1] = 1
                self.map[agent.position[0], agent.position[1]] = 0
                agent.position = (agent.position[0], agent.position[1] - 1)

        if action == ACTIONS['RIGHT']:
            if agent.position[1] + 1 < self.size and self.map[agent.position[0], agent.position[1] + 1] == 0:
                self.map[agent.position[0], agent.position[1] + 1] = 1
                self.map[agent.position[0], agent.position[1]] = 0
                agent.position = (agent.position[0], agent.position[1] + 1)
    ...
    def agent_capture(cls, pixel, size, map):
        potential_neighborhood = [(pixel[0] + 1, pixel[1]), (pixel[0] - 1,
                                                             pixel[1]), (pixel[0], pixel[1] + 1), (pixel[0], pixel[1] - 1)]
        n_agent_neighbour = 0
        for neighbour in potential_neighborhood:
            if neighbour[0] >= 0 and neighbour[0] < size and neighbour[1] >= 0 and neighbour[1] < size:
