    ...
    def test_position(self):
        ...
        for i in range(len(envs)):
            ...
            while not (done or lim < 500):
                self.check_position_unicity(envs[i].world)
    ...
    def test_reward(self):
        ...
        for i in range(len(envs)):
            ...
            while not (done or lim < 500):
                ...
                action = envs[i].action_space.sample()
                _, reward, _, _, _ = envs.step(action)
                score += reward
                var_targets = envs[i].world.capturedTargets - prev_targets
                var_mobiles = envs[i].world.capturedMobiles - prev_mobiles
                assert reward == (-1 + var_targets + 2 * var_mobiles)

            assert int(done) * score == int(done) * (
                -500 + envs[i].world.nb_targets + 2 * envs[i].world.nb_mobiles
            )
